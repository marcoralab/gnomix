...
if (exists("snakemake")) {
  in_q <- snakemake@input[["q"]]
  in_fam <- snakemake@input[["fam"]]
  in_pops <- snakemake@input[["pops"]]
  out_smap <- snakemake@output[["smap"]]
...
out_admix |>
  ...
  write_tsv(out_smap) |>
  pull("#Sample") |>
  write_lines(out_samplist)

save(out_admix, heatmap_mat, assign_cname_vec, assign_cname_vec_i,
  superpops, cluster_cols, filter_output, file = out_rda)
